import {
  ...
  export const createRecipe = createAsyncThunk(
    ...
    async (recipeData, { getState, rejectWithValue }) => {
      try {
        const response = await axiosInstance.post('/add', JSON.stringify(recipeData))
        return response.data;
      } catch (error) {
        return rejectWithValue(error.message);
      }
  ...
  export const fetchFilteredRecipes = createAsyncThunk(
    ...
    async (ingredients, { getState, rejectWithValue }) => {
      try {
        const response = await axiosInstance.post("/recipes/search", JSON.stringify({ingredients, "searchAll": true}));
        return response.data;
      } catch (error) {
        return rejectWithValue(error.message);
      }
...
export const updateRecipe = createAsyncThunk(
  ...
  async ({ id, ...updatedData }, { getState, rejectWithValue }) => {
    try {
      const response = await axiosInstance.put(`/user/recipe/update/${id}`, JSON.stringify(updatedData));
      if (!response.data) {
